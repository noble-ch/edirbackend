...
class ReminderViewSet(viewsets.ModelViewSet):
    ...
    def send_now(self, request, pk=None):
        ...
        if reminder.channel not in ['sms', 'all']:
            return Response(
                {'error': 'This reminder is not 78 configured for SMS'},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        success = reminder.send_sms()
        
        if success:
            return Response(
                {'status': 'sent', 'message': 'SMS reminders sent successfully'},
                status=status.HTTP_200_OK
            )
        else:
            return Response(
                {'status': 'failed', 'message': 'Failed to send SMS reminders'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def send_monthly_reminders(self, request, edir_slug=None):
        ...
        edir_slug = self.kwargs.get('edir_slug') if edir_slug is None else edir_slug
        edir = get_object_or_404(Edir, slug=edir_slug)
        ...
        if not unpaid_members.exists():
            return Response(
                {'status': 'completed', 'message': 'All members have paid for this month'},
                status=status.HTTP_200_OK
            )
        ...
        reminder = Reminder.objects.create(
            ...
            message=f'Please pay your monthly contribution of 500 birr for {today.strftime("%B %Y")}',
        ...
        if success:
            ...
            return Response(
                {'status': 'failed', 'message': 'Failed to send reminders'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def send_now(self, request, edir_slug=None, pk=None):
        ...
        if success:
            return Response(
                {
                    'status': 'sent',
                    'message': f'Reminders sent to {reminder.message} members',
                    'reminder_id': reminder.id
                },
                status=status.HTTP_200_OK
            )
